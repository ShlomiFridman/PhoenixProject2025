...
Original file is located at
    https://colab.research.google.com/drive/1A7Q6FlKJ-i-3yu_3eNittg_hic0aOz4h
...
class ChatbotService:
  ...
  def __initBot(self):
    ...
    patterns = [
        # (r"^what is (\w+)$", [self.term_summaries.get("%1", "Term not found")]),
        (r"^what is (\w+)", ["%1"]),
        (r"^explain (\w+)", ["%1"]),
        (r"^(\w+)$", ["%1"]),
        # (r"What is (.*)\??", [lambda matches: matches.group(1)]),
  ...
  def __getTerm(self, stmt):
    stmt = stmt.lower().strip()
    if stmt in self.term_summaries:
      return self.term_summaries[stmt]
    return "Term unknown"
  ...
  def __perform_send(self, q):
        ...
        results = self.chatbot.respond(msg)
        # results = self.__getTermSummery("What is (.*)\??", "what is kafka")
        # Display results
        with self.results_output:
            self.results_output.clear_output()
            if not results:
